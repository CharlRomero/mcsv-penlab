...
export const updatePasswordController = async (req, res) => {
  ...
  if (!userId || !newPassword) {
    logger.warn(
      "Intento fallido de actualización de contraseña: faltan campos obligatorios."
    );
    return res
      .status(400)
  ...
  try {
    ...
    logger.info(
      `Contraseña actualizada exitosamente para el usuario con ID: ${userId}.`
    );
    ...
    // Log de error si ocurre un problema
    logger.error(
      `Error actualizando la contraseña del usuario con ID ${userId}: ${error.message}`
    );
    res
      .status(500)
